...
\par\noindent\begin{supertabular}{@{}p{\lensymb}@{}p{\lenwhat}@{\hspace{\lencsep}}p{\lendefn}}
    ...
    $\omega$        & Crank angular velocity, \rad\per\second,                      & Eq.~(\ref{eq:?})                  \\
\end{supertabular}
...
\section{Model Formulation}
    ...
    Moreover, the piston position $x$ with respect to the TDC, in \meter, is expressed as%
    %
    \begin{equation}
        \label{eq:x}
        x(\alpha) = \left|
    ...
    \end{equation}%
    %
%    \noindent and  influences  the  thermodynamic  system  \emph{instantaneous}  volume  $V$.  For
%    constant-$\omega$ cases, one has:
%    %
%    \begin{align}
%        V(\alpha) &= \vtdc + \frac{\pi D^2}{4}x(\alpha) \quad\rightharpoondown \\
%        \label{eq:Va}
%            &= \vtdc + \frac{\pi D^2}{4}\left[
%                L\left(1 - \sqrt{1 - (\sin\alpha / r_{LR})^2}\right)+
%                    R\left(1 - \cos\alpha\right)
%                \right]; \quad\rightharpoondown \\
%        \label{eq:Vt}
%        V(t) &= \vtdc + \frac{\pi D^2}{4}\left[
%                L\left(1 - \sqrt{1 - (\sin(\omega t) / r_{LR})^2}\right)+
%                    R\left(1 - \cos(\omega t)\right)
%                \right].
%    \end{align}
%
