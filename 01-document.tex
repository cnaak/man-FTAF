...
\begin{tabbing}\hspace*{\lensymb}\=\hspace*{\lenwhat}\=\hspace*{\lendefn}\kill
    ...
    \tcs{\delta}            \> \tcW{Angular duration of combustion, \rad,}                                                              \> \tcd{Eqs.~(\ref{eq:delta}), (\ref{def:y})}       \\
    ...
    \tcs{\theta}            \> \tcW{Ignition angle, \rad,}                                                                              \> \tcd{Eqs.~(\ref{eq:theta}), (\ref{def:y})}       \\
...
\begin{tabbing}\hspace*{\lensymb}\=\hspace*{\lenWHAT}\kill
    ...
    \tcs{{}_i}                                  \> process index                                                                                                                            \\
    \tcs{{}_{\ell}}                             \> thermodynamic cycle index                                                                                                                \\
    ...
    \tcs{{}_{\delta}}                           \> at end of combustion reaction                                                                                                            \\
    \tcs{{}_{\theta}}                           \> at ignition                                                                                                                              \\
\end{tabbing}
...
\section{Model Formulation}\label{sec:model}
    ...
    Let $t_{\theta, \ell}$ be ignition times for which Eq.~(\ref{eq:theta}) holds true for the $\ell$-th thermodynamic engine cycle, then the $\ell$-th combustion event finishes at%
    %
    \begin{equation}
        t_{\delta, \ell} = t_{\theta, \ell} + \Delta t_c.
        \label{eq:t.delta}
    \end{equation}
    ...
    \end{equation}
    %
    \noindent one gathers enough equations as to perform the molar balance between products and reagents. Brunetti~\cite{2012-BrunettiF-Blucher} employs $K(\unit{1740}{\kelvin}) = 3.5$.  Using
    the same method, Table~\ref{tab:molBal} lists balanced reaction product coefficients.
    ...
    Plugging in Table~\ref{tab:molBal} results for $\phi > 1$ onto Eq.~(\ref{eq:K}) yields a quadratic equation for $\NCO$ whose solution may be written as:%
    %
    \begin{equation}
        \label{eq:NCO}
        \frac{\NCO}{\NF}   = -\beta \pm \sqrt{\beta^2 - \gamma},
    \end{equation}
    %
    \noindent with%
    %
    \begin{equation}
        \label{eq:gamma}
        \gamma          = \dfrac{2K\NC(\phi-1)}{\phi\epsilon(K-1)},
    \end{equation}
    %
    \noindent and%
    %
    ...
    \begin{align}
        \zeta(P, r) &= 17.807 + 6.423\mathtt{g}(r) \nonumber\\
                    ...
                    &\,+ [1.828 + 0.798\mathtt{g}(r)] \times 10^{-5} \times P^2,
        \label{eq:zeta}
    \end{align}
    %
    \noindent with%
    ...
    The reference FTHA concept of cumulative heat release fraction, $y(\alpha)$ is adapted in the present FTAF into a cumulative fraction of combustion reaction completion,  $y(\alpha)$,  with
    $0   \leqslant   y(\alpha)   \leqslant   1$,   with   $g(\alpha)$   being   its   functional   form---a   functional   model   parameter   that   can   follow   literature   proposals   of
    cossenoidal~\cite{2008-CurtoRissoPL+HernandezAC-JApplPhys}, and of Wiebe~\cite{2013-MartinsJJG-Publindustria} (exponential) forms---so that $g(\theta) = 0$, $g(\theta+\delta)  =  1$,  with
    $g(\alpha)$ being monotonic in the $\theta \leqslant \alpha \leqslant \delta + \theta$ interval---$\theta$ is the ignition angle, so that combustion starts whenever $\alpha = \theta$,  and
    $\delta = \omega\Delta t_c$ is the angular duration of the combustion, and $\Delta t_c$ is the time duration of the combustion process---so that%
    %
    \begin{equation}
        y(\alpha) =
        \begin{cases}
    ...
    %
    \begin{equation}
        Q_i + (U^0_{f,m,i} - U^0_{f,m,i+1}) - W_i   = U^0_{m,i+1} - U^0_{m,i},
        \label{eq:EnBal.2}
    \end{equation}
    %
    ...
    %
    \begin{equation}
        Q_i + \Delta{U_{reac,i}} - W_i              = U^0_{m,i+1} - U^0_{m,i},
        \label{eq:EnBal.3}
    \end{equation}
    %
    ...
    %
    \begin{equation}
        U^0_{m,i+1} = U^0_{m,i} + Q_i + \Delta{U_{reac,i}} - W_i,
        \label{eq:evol}
    \end{equation}
    %
    ...
    convenient function to invert $U^0_{m,i+1}(T)$ thus providing the value of $T_{i+1}$,  together  with  fact  that  the  system  volume  and  mass,  $V_{i+1}$  and  $m_m$,  are  known  from
    ...
    \begin{enumerate}
        ...
        \item Solving for $U^{0,j}_{m,i+1}$ using Eq.~(\ref{eq:evol}), whose inverse gives $T_{i+1}^j$; then
...
%    \end{figure*}
    ...
    Eqs.~(\ref{eq:mf})--(\ref{eq:mEoS}), and~(\ref{eq:U.mix})--(\ref{eq:U0.mix}).
