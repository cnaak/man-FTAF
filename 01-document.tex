...
\section{Model Formulation}\label{sec:model}
    ...
    \end{equation}
    %
    \noindent meaning that if its right-hand-side is known---$Q_i$ can be made zero by hypothesis, for simplicity, recalling that the model is  not  an  air-standard  one,  and  $Q_i$  is  not
    directly related to the combustion reaction---then Eq.~(\ref{eq:evol}) can be used to \emph{evolve} the system to the next, $i+1$, state, as the model implementation, shown below, provides
    a convenient function to invert $U_{0,m,i+1}$ thus providing the value of  $T_{i+1}$,  together  with  fact  that  the  system  volume  and  mass,  $V_{i+1}$  and  $m_m$,  are  known  from
    Eqs.~(\ref{eq:Va}) and~(\ref{eq:mfy}), with an additional state function (system intensive property) given by%
    %
    \begin{equation}
        \label{eq:v}
        v_{m,i} = \frac{V_i}{m_m}.
