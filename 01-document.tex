...
\section{Model Formulation}\label{sec:model}
    ...
    The net work executed by the system during the $i$-th (sub-)process, $W_i$, is  obtained  by  integration  of  the  $i$-th  locally  polytropic  process,  $P_iV_i^{n_i}  =  \mathbb{C}_i  =
    P_{i+1}V_{i+1}^{n_i}$, thus yielding%
    %
    \begin{equation}
        W_i =
        \begin{cases}
            \displaystyle
            \frac{-P_i}{1-n_i}\left[V_i - V_{i+1}\left(\frac{V_i}{V_{i+1}}\right)^{n_i}\right], & \text{for } n_i \neq 1 \\
            \displaystyle
            -P_iV_i\ln\frac{V_i}{V_{i+1}},                                                      & \text{for } n_i = 1.
        \end{cases}
        \label{eq:Wi}
    \end{equation}
