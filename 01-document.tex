...
\section{Model Formulation}\label{sec:model}
    ...
    % TODO
    Let  $\zeta$  be  the  residual  fraction  of  gas  products  from  a  previous  cycle  that   remain   in   the   system   for   a   new   cycle.   Using   the   relationship   given   by
    Heywood~\cite{1988-HeywoodJB-McGrawHill}, i.e.,%
    %
    \begin{multline}
        \zeta(P, r) = 17.807 + 6.423g(r) - [0.029 + 0.013g(r)] P \\
                    + [1.828 + 0.798g(v)] \times 10^{-5} \times P^2,
        \label{eq:zeta}
    \end{multline}
    %
    \noindent with $g(v) = (5.25 - 0.5r) e^{(8.5 - r)}$, so that the following mixtures can be defined:
    %
    \begin{align}
        \MXAIR  &=  \NA\left(
                        \frac{1}{1+\psi}\mbox{\ce{O2}} +
        ...
        \MXAFU  &=  \NF\,\mbox{\ce{C\NC H\NH O\NO N\NN}} + \NA\left(
                        \frac{1}{1+\psi}\mbox{\ce{O2}} +
                        \frac{\psi}{1+\psi}\mbox{\ce{N2}}
                    \right) \\
    ...
    Therefore the reactive mixture that comprises the thermodynamic system at the beginning of a cycle is  $\MXREA$,  i.e.,  the
    reagents; while at the end of the cycle, $\MXPRO$, i.e., the products.
