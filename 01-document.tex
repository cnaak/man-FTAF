...
\section{Model Formulation}
    ...
    $V_\tdc$ and $V_\bdc$ for the top and bottom dead centers, respectively; the engine compression ratio $r$ is given by  $r  =
    V_\bdc / V_\tdc$.

    The cylinder diameter is $D$, the piston stroke is $S$, the rod length is $L$, and the crank radius is $R$, all  in  \meter.
    Important ratios include the stroke-to-diameter one, $r_{SD} = S/D$; the rod-to-crank one, $r_{LR} = L/R > 1$, so that%
    %
    \begin{align}
        \label{eq:S}
        S       &= 2 R = \frac{4 V_{du}}{\pi D^{2}}, \\
        \label{eq:Vdu}
        V_{du}  &= V_\bdc - V_\tdc = \frac{\pi r_{SD}D^3}{4}, \\
        \label{eq:r}
        r       &= 1 + \frac{V_{du}}{V_\tdc}.
    \end{align}
