...
\section{Model Implementation}
    ...
    (of precision) and \code{coef}, respectively, is exemplified below. The $a_i$ coefficients inside \code{coef} are renamed as to ease code inspections.
...
struct cpFC # cp Function Coefficients
   prec::AbstractFloat
   coef::NamedTuple{(:ap0,:ap1,:ap2,:ap3,:ap4,:an1,:an2,:an3,:an4),NTuple{9,Real}}
end
        \end{lstlisting}
        \caption{Listing for the \code{cpFC} definition code snippet, for storing $\bar{c}_p\,:\,\bar{c}_p(T)$ model coefficients}
    \end{figure*}
    ...
    numerics~\cite{2011-TuckerW-Princeton-Validated, }, which conforms  to
    the IEEE Std 1788-2015~\cite{2015-RevolN+ThompsonT-IEEE-IA}.

    A fluid library, \code{FL}, can be built by storing in a container values of the proposed type \code{bProp}, basic properties, exemplified below.
