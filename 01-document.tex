...
\begin{tabbing}\hspace*{\lensymb}\=\hspace*{\lenwhat}\=\hspace*{\lendefn}\kill
    ...
    \tcs{Q}                 \> \tcw{Heat interaction to system, \kilo\joule,}                                                           \> \tcd{Eq.~(\ref{eq:EnBal})}                       \\
    ...
    \tcs{W}                 \> \tcw{Work interaction to system, \kilo\joule,}                                                           \> \tcd{Eq.~(\ref{eq:EnBal})}                       \\
...
\section{Model Formulation}\label{sec:model}
    ...
    %
    \begin{align}
        Q_i - W_i                                   &= U_{m,i+1} - U_{m,i}      \qquad\rightharpoondown \\
        Q_i + (U^0_{f,m,i} - U^0_{f,m,i+1}) - W_i   &= U_{0,m,i+1} - U_{0,m,i}  \qquad\rightharpoondown \\
        Q_i + \Delta{U_{reac,i}} - W_i              &= U_{0,m,i+1} - U_{0,m,i},
        \label{eq:EnBal}
    \end{align}
    %
    \noindent which clearly shows the distiction between heat transfer interactions (for instance, with the engine  block),  and
    internal energy generation due to chemical reaction, in which
    %
    \begin{align}
        \Delta{U_{reac,i}}  &\equiv U^0_{f,m,i} - U^0_{f,m,i+1}     \qquad\rightharpoondown \\
                            &= H^0_{f,m,i} - \mathrm{n}_{m,i}\bar{R}T_0 - H^0_{f,m,i+1} + \mathrm{n}_{m,i+1}\bar{R}T_0.
        \label{eq:dUreac}
    \end{align}

    It is worth noting that $-\bar{R}T_0(\mathrm{n}_{m,i} - \mathrm{n}_{m,i+1})$ does not vanish in general because quantity  of
    matter, contrary to mass, is \emph{not} conserved during chemical reactions.
